...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.checkCodeOfConductSafePractice();
  ...
  //// CodeOfConductDeclaration
  checkCodeOfConductDeclaration() {

    if (this.dataService.getStoredData('xyz1234xy_codeOfConductAgreementsV080620') === '1') {
      return;
    }
  ...
  acceptCodeOfConductDeclaration() {
    ...
    this.dataService.Post('', 'acceptCodeOfConductDeclaration').subscribe(
      res => {
        ...
        if (res === 1) {
          this.dataService.StoreData('xyz1234xy_codeOfConductAgreementsV080620', '1');
          this.utilityService.showSuccessToast('Thank you.', 'Success');
  ...
  //// CodeOfSafePractice
  checkCodeOfConductSafePractice() {

    if (this.dataService.getStoredData('xyz1234xy_codeOfConductSafePractice') === '1') {
      return;
    }
    ...
    this.dataService.Get('checkCodeOfSafePractice').subscribe(
      res => {

        if (res !== 1) {
          this.showCodeOfSafe = true;
        }
      },
      error => {
        this.showCodeOfSafe = true;
  ...
  acceptCodeOfConductSafePractice() {
    this.isInprogress = true;
    this.dataService.Post('', 'acceptCodeOfSafePractice').subscribe(
      res => {

        if (res === 1) {
          this.dataService.StoreData('xyz1234xy_codeOfConductSafePractice', '1');
          this.utilityService.showSuccessToast('Thank you.', 'Success');
          this.showCodeOfSafe = false;
        }
  ...
  getTop6LinksLocally() {
    if (!!this.dataService.getStoredTempData('xyz123xyz_top6LinksLocally')) {
      ...
      const corpLink = this.top6Links.filter(x => x.name === 'WEMA ANALYTICS');
      const objCorp = corpLink[0];
      objCorp.url = `${objCorp.url}?id=${this.currentUser.company}`;

      let newLink = [];

      this.top6Links.map(x => {
        if (x.name !== 'WEMA ANALYTICS') {
          newLink.push(x);
        } else {
          newLink.push(objCorp);
        }
      });

      this.top6Links = newLink;
  ...
  checkAnalytics() {
    ...
    const staffId = this.currentUser.company;

    this.dataService.Get(`analytics/exco?email=${email}`).subscribe(
      res => {
        console.log('res.resp:', res);
        if (!res.error) {
          this.analyticsUrl = res.analyticsPage.page_url + `/?id=${staffId}`; // `http://iwema:8089/analytics/?id=${staffId}`;
          this.analyticsMessgae = res.analyticsPage.message;
          this.showAnaytics = true;
        } else {
          this.showAnaytics = false;
        }
      });
  }
