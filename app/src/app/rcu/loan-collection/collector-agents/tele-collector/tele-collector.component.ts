...
export class TeleCollectorComponent implements OnInit {
  ...
  ngOnInit() {
    // const myAccess = this.utilityService.getAccessInfo('RC-Loan Tele Collector');
    ...
    this.agentId = this.dataService.getStoredTempData(`TeleAgent_${this.agentEmail.toLowerCase()}`);
  ...
  getCaseCollections = (caseEndPoint, forceRefresh: boolean = false) => {
    ...
    if (caseEndPoint === caseEndpoint.teleCollectorCases) {
      ...
      endPointUrl = `tele-collector/loanCollections?agentId=${this.agentId}&selectedCase=${caseEndpoint.flagedCases}`;
    }
  ...
  getAgent = async () => {
    const url = `getAgentByEmail?category=Tele Collector`;
    this.dataService.Get(url).subscribe(
      res => {

        if (res && res.data) {
          this.agentId = res.data.Id;
          sessionStorage.setItem(`TeleAgent_${res.data.Email.toLowerCase()}`, res.data.Id);
