...
export class ReportTemplateComponent implements OnInit {
  ...
  @Input() useReportManager = false;
  @Input() downloadClick = false;
  ...
  @Output()
  downloadTriggered: EventEmitter<void> = new EventEmitter<void>();
  ...
  @Input() searchText1_Placeholder = '';
  @Input() searchText1_width = 150;

  @Input() searchText2_Placeholder = '';
  @Input() searchText2_width = 150;

  @Input() searchText3_Placeholder = '';
  @Input() searchText3_width = 150;

  @Input() searchText4_Placeholder = '';
  @Input() searchText4_width = 150;
  ...
  @Output()
  searchDownloadTriggered: EventEmitter<object> = new EventEmitter<object>();
  ...
  getCode(x) {
    if (_.get(this.reportHeaders[x], 'isDate')) {
      ...
      return 4;
    } else if (_.get(this.reportHeaders[x], 'isCurrency')) {
      return 5;
    } else {
      return 1;
    }
  ...
  onSearchDownload() {
    ...
    const params = {
      ...
      sText3: this.searchText3,
      sText4: this.searchText4,
      per_page: this.per_page,
      lowerAccess: this.myAccess.key === 'B' || this.myAccess.key === 'S' ? true : false
    };

    this.searchDownloadTriggered.emit(params);
  }
  ...
  onDownloadRow(selectedRow) {
    this.downloadTriggered.emit(selectedRow);
  }
