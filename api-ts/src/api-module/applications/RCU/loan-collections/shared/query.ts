...
export const getAllLoanCollectionsQuery = () => {
    return `select * from custom.loanpdorpt`;
};

export const getLoanCollectionsQuery = (accountNos: string[]) => {
    return `select * from custom.loanpdorpt where FORACID in (${accountNos.map(acc => `'${acc}'`).join(',')})`;
};



export const getLoanCollections2 = () => {
    return `select
...
export const getAllLoanQuery = () => {
    ...
    (select dmd_amt from tbaadm.ldt l,tbaadm.gam g where l.acid = g.acid and g.acid=a.op_acid and dmd_flow_id = 'INDEM') interest_due,
    (select dmd_amt from tbaadm.ldt l,tbaadm.gam g where l.acid = g.acid and g.acid=a.op_acid and dmd_flow_id = 'PRDEM') principal_due,
    dis_shdl_date start_date, dis_amt loan_amount from tbaadm.lam a where rownum <= 100`;
};
