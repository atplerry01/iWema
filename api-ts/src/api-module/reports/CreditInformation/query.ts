export const getCreditInformation_QUERY = (callDate) => {
    ...
    (select case Lr_freq_type WHEN 'W' THEN 'WEEKLY' WHEN 'H' THEN 'HALF YEARLY' WHEN  'M' THEN 
    'MONTHLY' WHEN 'Q' THEN 'QUATERLY' WHEN 'Y' THEN 'YEARLY' WHEN 'F' THEN 'FORTHNIGHTLY' END from Tbaadm.lrs where acid = a.acid and rownum=1) "RepaymentFrequency",
    ...
    from custom.provision_details_i where curr_class_date = '${callDate}' AND BALANCE < 0) c, tbaadm.gam a, tbaadm.lam b, Tbaadm.sdr e
    where c.acid = a.acid(+)
    and a.acid = b.acid(+)
    and b.acid = e.acid(+)
    ...
    to_char((Select flow_amt from tbaadm.lrs x where x.acid = a.acid and rownum=1)) "Installment amount",
    a.Acct_crncy_code "Currency",
    c.dpd "DaysInArrears",
