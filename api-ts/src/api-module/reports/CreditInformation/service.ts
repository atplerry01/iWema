...
import { getCreditInformation_QUERY } from "./query";
import { executeFinacleDRQuery } from "../../../db/dbUtil";

export const getCreditInformation = async (callDate, accessLevels, redis: any, page, per_page) => {

    return new Promise(async (resolve, reject) => {

        const level = await checkAccess('CBR-Credit Information', accessLevels);
        if (!level) {
            return reject({
                err: "",
        ...
        if (!callDate) {
            ...
            const key = `getCreditInformation${callDate}`;
        ...
        try {         
          ...
          const q =  getCreditInformation_QUERY(callDate);                  
