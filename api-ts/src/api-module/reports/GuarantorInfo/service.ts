...
import { getGuarantorInfo_QUERY } from "./query";
import { executeFinacleDRQuery } from "../../../db/dbUtil";

export const  getGuarantorInfo = async(callDate, accessLevels, redis: any, page, per_page) => {

    return new Promise(async (resolve, reject) => {

        const level = await checkAccess('CBR-Guarantors Information', accessLevels);
        if (!level) {
            return reject({
                err: "",
        ...
        if (!callDate) {
            ...
            const key = `getGuarantorInfo${callDate}`;
        ...
        try {         
          ...
          const q =  getGuarantorInfo_QUERY(callDate);                  
