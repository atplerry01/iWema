...
// oracle  // staffId = 'S08166' => relationship manager  //FD
export const getTotalFDSum_QUERY = (staffId: string, period: string) => {
    const dt = new Date();
    ...
    return `select sum(clr_bal_amt * CUSTOM.AmountToFC('NOR','${dateTo}',gam.acct_crncy_code,'NGN')) ACTUAL_BAL,  
    sum (custom.getAvgBal_cr(gam.acid,'${dateFrom}','${dateTo}') * CUSTOM.AmountToFC('NOR','${dateTo}',gam.acct_crncy_code,'NGN')) AVERAGE_BAL
